...
void GetParams(int* n_cameras, int* n_images, int* buffer_size, char* format){
	json cfg;
	...
	*n_images = (int)cfg["n_images"] / *buffer_size;
	std::string format_ = cfg["format"];
	memcpy((void*)format, (void*)format_.c_str(), sizeof(format));
}
...
int main(int argc, char* argv[])
{
	...
	for(int N = 0; N < N_CAMERAS; N++){
		cam[N] = new CameraDaheng(N+1, CHANNEL_NUM);
		...
		#pragma omp parallel
		{
			...
			for(int N = 0; N < N_CAMERAS; N++){
				for (int i = 0; i < BUFFER_SIZE; i++){
					...
					WriteImg((void*)&write_args[N][i], format);
				} 
			}	
		}		
	}
	...
	printf("USER_API: Time elapsed (%s): %f seconds.\n", format, end-start);
